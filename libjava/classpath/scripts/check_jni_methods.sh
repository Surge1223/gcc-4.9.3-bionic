...
grep -h '^JNIEXPORT .* Java_' /home/surge/Videos/GCC.git/libjava/classpath/include/*.h | \
...
find /home/surge/Videos/GCC.git/libjava/classpath/native/jni -name \*.c | \
	xargs grep -h '^Java_' | \
...
find /home/surge/Videos/GCC.git/libjava/classpath/native/jni -name \*.cpp | \
	xargs grep -h '^Java_' | \
        LC_ALL=C sed -e 's,^\(Java_[a-z_A-Z0-9]*\).*$,\1,' >> $TMPFILE2
find /home/surge/Videos/GCC.git/libjava/classpath/native/jni -name \*.cpp | \
